...
def summary_card(aircraft, dep, arr):
    ...
    route_row = (
        f'<div class="sc-meta">'
        f'<span style="font-size:.55rem;letter-spacing:.16em;text-transform:uppercase;color:#b8a898;margin-right:6px">Route</span>'
        f'<span class="sc-route-val">{city(dep)}{code(dep)}</span>'
        f'<span class="sc-sep">→</span>'
        f'<span class="sc-route-val">{city(arr)}{code(arr)}</span>'
        f'</div>'
    ) if (dep or arr) else ""

    rows = "".join(
        f'<div class="sc-row">'
        f'<span class="sc-idx">{i:02d}</span>'
        f'<span class="sc-acname">{H.escape(ac["name"])}</span>'
        f'<span class="sc-price">{H.escape(ac["price"])}</span>'
        f'<span class="sc-ft">{H.escape(ac["ft"])}</span>'
        f'</div>'
        for i, ac in enumerate(aircraft, 1)
    )
    ...
    return (
        ...
        f'<button class="copy-btn" id="copybtn" data-text="{data_text}" '
        f'onclick="var t=JSON.parse(this.getAttribute(\'data-text\'));navigator.clipboard.writeText(t).then(()=>{{this.textContent=\'✓ Copied!\';this.classList.add(\'copied\');setTimeout(()=>{{this.textContent=\'Copy\';this.classList.remove(\'copied\')}},2500)}});">'
        f'Copy</button>'
        f'</div>'
        f'{route_row}'
...
def render_cards(content, session_id="", all_aircraft=None, rdep="", rarr="", is_last=False, is_first=False):
    ...
    if is_first:
        ...
        if intro_m:
            cleaned = clean_intro(intro_m.group(1))
            if cleaned.strip():
    ...
    strip = (
        ...
        f'<span class="fs-meta">Top options by price</span>'
        f'</div>'
    ) if (dep_short != "—" or arr_short != "—") else ""

    cards = ""
    for idx, ac in enumerate(aircraft, 1):
        route_html = (
            f'<div class="ac-route">'
            f'<span class="route-label">Route</span>'
            f'<span class="route-airport">{H.escape(ac["dep"] or "—")}</span>'
            f'<span class="arrow">→</span>'
            f'<span class="route-airport">{H.escape(ac["arr"] or "—")}</span>'
            f'</div>'
        ) if (ac["dep"] or ac["arr"]) else ""
        cards += (
            f'<div class="aircraft-card">'
            ...
            f'</div>{route_html}</div>'
        )

    sum_html = ""
    if is_last and session_id:
        sum_ac = all_aircraft or aircraft
        sum_html = summary_card(sum_ac, strip_dep, strip_arr)
...
def build_chat_html(messages, is_loading, session_id=""):
    ...
    for i, m in enumerate(messages):
        ...
        if m["role"] == "user":
            ...
            inner = render_cards(m["content"], session_id, sess["all_ac"], sess["dep"], sess["arr"], is_last_in_sess, is_first_in_sess)
...
if st.session_state.is_loading:
    ...
    if last:
        try:
            ...
            reply = "⚠ Cannot connect to backend. Run: uvicorn main:app --reload"
        except requests.exceptions.Timeout:
            reply = "⚠ Search timed out — Avinode may be slow. Please try again."
